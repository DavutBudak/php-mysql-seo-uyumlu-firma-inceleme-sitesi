(function($) {
	var
		...
		is__mobile = function() {
			if(picker&&pickers[picker.id].&&!pickers[picker.id].mobile) {
				if($(window).width()<480)
					picker.element.removeClass('picker-s');
				else
					picker.element.addClass('picker-s')
			}
		},
		...
		return $(this).each(function(){
			if($(this).is('input')&&!$(this).hasClass('picker-input')) {
				...
				var
					...
					picker_ = (input.data('')===false) ? input.data('') : true,
					picker__class = (input.data('')===false) ? '' : 'picker-s',
					picker__mobile = (input.data('-mobile')===false) ? input.data('-mobile') : true,
				...
				$('<div>', {
					class: 'datedropper ' + picker_modal + ' ' + picker_theme + ' ' + picker__class + ' ' + picker_large_class,
					id: id,
					html: $('<div>', {
						class: 'picker'
					})
